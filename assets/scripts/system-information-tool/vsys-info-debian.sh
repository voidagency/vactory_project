...
if $(dpkg --compare-versions $currentAPACHEVersion "ge" "2.4"); then
...
if $(dpkg --compare-versions $currentMYSQLVersion "ge" "5"); then
...
if $(dpkg --compare-versions $MYSQLCONFIGVALUE "ge" "8192"); then
...
if $(dpkg --compare-versions $currentPHPVersion "ge" "7"); then
...
if $(dpkg --compare-versions $PHPINIVALUE "ge" "2000"); then
...
if $(dpkg --compare-versions $PHPINIVALUESUB "ge" "128"); then
...
PHPINIVALUESUB=${PHPINIVALUE:0:${#PHPINIVALUE}-1}
if $(dpkg --compare-versions $PHPINIVALUESUB "ge" "256"); then
...
if $(dpkg --compare-versions $PHPINIVALUE "ge" "180"); then
...
PHPINIVALUESUB=${PHPINIVALUE:0:${#PHPINIVALUE}-1}
if $(dpkg --compare-versions $PHPINIVALUESUB "ge" "20"); then
...
if $(dpkg --compare-versions $DRUSHVersion "ge" "8"); then
